    ...
    <UDashboardPanel>
      <UDashboardNavbar title="Users">
        <template #right>
          ...
          <UInput ref="input" v-model="search" icon="i-heroicons-magnifying-glass" autocomplete="off" placeholder="Search..." @keydown.esc="$event.target.blur()">
            <template #trailing>
              <UKbd value="/" />
            </template>
          </UInput>
      ...
      <UTable
        ...
        sort-mode="manual"
        class="w-full"
        :ui="{ thead: 'sticky top-0 bg-background z-[1] border-b border-gray-200 dark:border-gray-800', divide: 'divide-gray-200 dark:divide-gray-800', td: { base: 'max-w-[0] truncate' } }"
        @select="select"
      >
